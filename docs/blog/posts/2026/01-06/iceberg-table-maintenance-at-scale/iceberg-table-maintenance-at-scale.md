...
## Control plane 的中心正在收斂到 Iceberg REST Catalog
...
Netflix 在2023年的演講提到，他們用的是內部的 **Metacat** 當 metadata 抽象層，比 Iceberg REST Catalog spec 出現得早：federated metadata store （整合之前的Hive Metastore, customized Polaris Iceberg REST Catalog, 其他druid 和snowflake 的資料） + 統一 API + 統一 type system，讓公司內部的人可以統一用同一套metadata system without 真的需要知道底層的資料儲存細節。所有 maintenance 服務（Autotune、Janitor、Autolift）都跑在 Metacat 之上，透過開源 Iceberg API 操作 metadata。從架構上看，Metacat 在 Netflix 系統裡承擔的就是「catalog 兼 control plane」這個角色。Iceberg REST Catalog spec 出現之後，Netflix 並沒有立刻全面轉過去，而是讓 Metacat 跟 REST Catalog 並存。
...
### Apple 把 Iceberg REST Catalog 擴充成 TMS 的 source of truth
