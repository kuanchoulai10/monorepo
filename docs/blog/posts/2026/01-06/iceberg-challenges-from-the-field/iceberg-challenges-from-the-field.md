...
Iceberg 已經是現在 open table format 的事實標準。從 Snowflake、Databricks、AWS 到 BigQuery，幾乎所有主流分析平台都把 Iceberg 列為支援對象；社群裡的規範討論、提案與各種整合工作也維持著高頻率產出。
...
- [High-Performance Full Text Search Directly on Iceberg via Lucene-Integrated Indexing](https://youtu.be/KO04A2uEa0E)，由 StarTree.ai 的 Xiang Fu 與 Raghav Yadav 共同主講，討論在 Iceberg 之上直接支援全文檢索

三場 talks 的主題乍看沒有什麼共通之處。但仔細研究之後會發現，三家公司投入的工程資源高度重疊：都在同樣幾個層面自行研發 Reader、加 cache、把單機 metadata planning 改成分散式 metadata planning。

這篇文章的目的，是把這三場 talks 放在一起仔細研究。先看三家公司各自要解的問題，再萃取它們不約而同投入的共同 patterns，最後從這些 patterns 反推 Iceberg 目前還沒解決的設計不足。

## 三場 Talks 指向同一組挑戰

要解釋為什麼這三場 talks 值得放在一起仔細研究，需要先把三家公司各自的定位看清楚。它們在 OLAP 領域起點不同、產品形態也不同，但都同時把 Iceberg 作為近期的重點整合對象：

- **ClickHouse** 是 open-source columnar OLAP database，以極低查詢延遲和高吞吐量寫入著稱，常被用在 observability、event analytics 等 append-heavy 場景。原本以自家儲存格式為主，最近一年半開始正式投入 Iceberg 整合。
- **Firebolt** 是面向工程團隊的 cloud-native analytical database（Postgres 相容），主打 sub-second 查詢、高並發、price-performance，常用來支撐 customer-facing analytics 與 data application backend。它把 Iceberg 應用在 trillion-row 規模與上萬欄位的 wide table 場景。
- **StarTree.ai** 是 Apache Pinot 的商業化平台，主打 streaming data 上的 sub-second 與高並發 user-facing analytics。它把 Iceberg 與全文檢索結合在一起，讓使用者不必再額外維護一套 Elasticsearch。

三家的產品形態、客戶群、原本擅長的場景都不同。但只要主題是「把 Iceberg 整合進產品時要解決什麼」，三場 talks 的內容立刻開始重疊。在 Iceberg 與底層 Parquet 的設計下，所有低延遲查詢產品都會在同樣幾個層面遇到相同的限制。

## 三場 Talks 各自在解什麼問題

把三家公司各自的整體取捨先看清楚，後面要歸納共同 patterns 時才不會把不同的問題混在一起。三場 talks 雖然都圍繞「在 Iceberg 上做查詢」這件事，但各自的起點與要解的關鍵問題完全不同。
...
所以 ClickHouse 過去一年半的整體取捨是：**所有原本由通用程式庫承擔的 I/O 與 metadata 邏輯，都改由 ClickHouse 自己實作**。用 C++ 重新實作 Iceberg 規範、為 Parquet 寫原生 Reader、為 metadata 與 footer 各自設計 cache 層。這些工作都把「整合 Iceberg 後仍要 sub-second」當作不可妥協的設計前提。
...
### StarTree：在不搬離 Iceberg 的前提下，把全文檢索做到亞秒級

StarTree 的問題定義跟前面兩家不太一樣。它的客戶要在同一份資料上同時做傳統的 OLAP 聚合與全文檢索類的查詢（fuzzy search、phrase query 等）。過去的做法是把資料同時寫進一個 OLAP 系統與一個 Elasticsearch，由應用層自己 join。這意味著兩套系統的維運、資料同步、一致性處理都要自己負責。

所以 StarTree 的整體取捨是：**在不搬離 Iceberg 的前提下，把全文檢索能力放回 Pinot 的查詢路徑裡**。具體做法是用一個 Watcher 任務監控 Iceberg table 的檔案目錄，每當新的 Parquet 檔案出現就建立一個輕量的 pseudo segment（只記錄檔案 URL），同時 in-line 為這個檔案產生 Lucene index，允許使用者把 index pin 在本地。查詢端就能用 Lucene index 先找出符合條件的記錄，再從 Iceberg 讀取對應的 page。

## 三家不約而同投入的共同 Patterns

三家公司的整體取捨不一樣，但實際做出的工程動作卻在同樣三個層面高度重疊。下面依序拆開來看每一個層面，以及每一家在這個層面上做了什麼。

### Pattern 1：S3 的 metadata 讀取延遲，三家都另加一層 cache 規避
...
三家做法的細節不同，但本質都是把 Iceberg metadata 從「跟 S3 同步、每次查詢都重新讀」改成「在本地 cache、按 freshness 規則更新」。Iceberg 規範並沒有規定 metadata 應該怎麼被讀取或 cache，這層是三家在自家系統裡自己補上的。

### Pattern 2：標準 Parquet Reader 控制力不足，三家都自行研發 Reader
...
第三個重疊層面是 metadata planning 本身的 scaling。Iceberg 規範描述了 metadata 的 layout，但沒有規定 query engine 該如何分配 planning 工作。多數實作的預設是「由一個 coordinator 節點讀完所有 metadata，再分發實際的 data read」。當 metadata 規模從幾百份 manifest 變成數萬份、數百萬個 data files，這個 coordinator 就變成 single point of bottleneck。
...
- **StarTree**：把每個 Parquet 檔案抽象成輕量的 pseudo segment（只記 URL 與 metadata），於是 Pinot 原本針對 segment 設計的 Broker → Server 兩層 pruning 架構可以直接套用：Broker 在分發查詢前先剔除不符合的 segment，Server 再做 file 內部的 predicate 過濾。整套 planning 自然就分散到 Broker 與 Server 上。

三家用了不同的抽象（replica、SQL table、pseudo segment），但解的是同一個問題：把原本集中在一個 coordinator 上的 metadata planning 拆解到多個節點上。Iceberg 規範本身對這件事沒有規定，三家都用自家既有的分散式機制各自重新設計了一套。

## 這些 Patterns 反推出的 Iceberg 設計不足

如果只是一家公司在某個層面上投入額外工程，那可以解釋為他們的場景特殊。但同一個層面上同時有三家公司、用三種不同抽象解決同一個問題，比較像是 Iceberg 規範本身在這幾個層面做得不夠。下面把這幾個不足整理出來，並區分：哪些是 Iceberg / Parquet 本身的設計限制、哪些其實是整合方面對自家場景的特殊需求。
...
這個問題不會因為換家公司就消失。任何想在 cloud object storage 上做低延遲查詢的產品都會面對相同的瓶頸。因此這算是 Iceberg metadata layout 在 cloud-native 場景下的設計限制，屬於規範層面的不足。

### 讀 Parquet 仍需大量優化，但 Iceberg 規範沒提供標準路徑
...
- file 內部 row group 級的任務切分不在 Iceberg 規範裡，一個巨大的 file 容易讓單一節點成為瓶頸
- 通用中介層（例如 Arrow）為了適配多種 query engine 而付出的格式轉換成本，在 sub-second 場景下會被放大

Iceberg 對 file 層級的 read pattern 給出了完整規範（manifest 列出 files、column stats 落在 file 或 row group level、partition pruning 與 column projection 都有對應規則），但 file 之內如何被讀取，幾乎沒有任何規定。這個範圍可以理解為規範刻意留白，也可以理解為不足。我傾向把它視為不足：cloud-native + sub-second 已經是查詢場景的主流，每家公司都各自重新設計一套 sub-file 讀取模型，對 Iceberg 生態而言這個工程成本相當顯著。
...
但 Puffin 的角色還在很早的階段。它在 spec 上是 optional 的，reader 可以選擇忽略，這意味著不同 query engine 對 Puffin 的支援程度落差很大。spec 明確列出的 blob type 也只有兩種，全文檢索、向量檢索、inverted index 這類其他類型的 index 沒有對應的標準化容器。想做 Lucene 之類的 index 時，整合方仍然得回到 StarTree 那套自己做的路徑：自己管理 index file、自己處理同步、自己定義跨 catalog 跨 query engine 的讀取協定。
